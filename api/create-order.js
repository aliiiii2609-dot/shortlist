...
export default async function handler(req, res) {
  ...
  // Two tiers now, so the button that opened this tells us which one. An
  // unknown or missing id fails closed rather than silently charging the
  // wrong amount for a guessed default.
  ...
  // Defensive floor, not a real limit at either of this app's price points:
  // Razorpay itself rejects anything under 100 paise, so fail with a clear
  // message before it ever reaches their API.
  ...
  try {
    r = await fetch("https://api.razorpay.com/v1/orders", {
      ...
      body: JSON.stringify({
        ...
        currency: "INR",
        receipt: `${plan.id}_${user.id.replace(/-/g, "").slice(0, 12)}_${Date.now().toString(36)}`,
        // Same trick as checkout.js: survives the round trip and comes back
        // on the webhook's payment entity, so attribution never depends on
  ...
  return res.status(200).json({
    ...
    key_id: key,   // safe to return: this is the public half, same value a VITE_ var would ship
